<?php
// ดึงข้อมูลจาก API
$apiUrl = "http://localhost:8080/csP1015/project/plant_diseases/api.php?page=" . ($_GET['page'] ?? 1);
     ...
     <div class="container-fluid">
                         ...
                         <table class="table table-bordered" id="diseaseTable">
                              <thead>
                                   <tr>
                                        ...
                                        <th>ชื่อโรค (ภาษาอังกฤษ)</th>
                                        <th>รูปตัวอย่างโรค</th>
                                        <th>จัดการ</th>
                                   </tr>
                              </thead>
                              <tbody>
                                   <?php foreach ($diseases as $index => $row): ?>
                                        <tr>
                                             ...
                                             <td><?php echo $row['disease_name_th']; ?></td>
                                             <td><?php echo $row['disease_name_en']; ?></td>
                                             <td>
                                                  <?php
                                                  if (isset($row['id']) && !empty($row['id'])) {
                                                       $disease_id = str_pad($row['id'], 5, '0', STR_PAD_LEFT);
                                                  } else {
                                                       $disease_id = "00000";
                                                  }

                                                  $imagePath = $row['image'] . "?v=" . time();
                                                  echo '<img src="' . $imagePath . '" alt="' . $row['disease_name_th'] . '" width="500">';
                                                  ?>
                                             </td>
                                             <td>
                                                  <a href="D1_detail.php?id=<?php echo $row['id']; ?>"
                                                       class="btn btn-info btn-sm"><i class="bi bi-file-text"></i></a>
                                                  <a href="D1_edit.php?id=<?php echo $row['id']; ?>"
                                                       class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i></a>
                                                  <button class="btn btn-danger btn-sm" onclick="deleteDisease(this)"
          ...
          <script>
               function deleteDisease(button) {
                    ...
                    if (!confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบ ID ${diseaseId} ?`)) {
                         return;
                    }

                    fetch("http://localhost:8080/csP1015/project/plant_diseases/api.php", {
                         method: "DELETE",
                         headers: {
